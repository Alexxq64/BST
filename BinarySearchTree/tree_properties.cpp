...
/**
 * @brief Вычисление контрольной суммы B-дерева (ДБД)
 * @param root Указатель на корень B-дерева
 * @return Сумма всех ключей в дереве
 *
 * @details
 * Рекурсивно вычисляет сумму всех ключей в B-дереве.
 * Алгоритм:
 * 1. Суммирует все ключи текущего узла
 * 2. Рекурсивно суммирует ключи всех дочерних узлов
 *
 * Используется для проверки корректности построения дерева.
 */
...
/**
 * @brief Вычисление взвешенной суммы уровней B-дерева
 * @param root Указатель на корень B-дерева
 * @param level Текущий уровень (начинается с 0 для корня)
 * @return Взвешенная сумма: ∑(уровень × количество_ключей_на_уровне)
 *
 * @details
 * Вычисляет сумму произведений уровня узла на количество ключей в нем.
 * Используется для вычисления средней глубины ключей в B-дереве.
 *
 * @note Уровень корня = 0 (или 1, в зависимости от определения)
 */
...
/**
 * @brief Вычисление высоты B-дерева (ДБД)
 * @param node Указатель на узел B-дерева
 * @return Высота дерева (количество уровней)
 *
 * @details
 * Высота B-дерева определяется как:
 * - 1 для листового узла
 * - 1 + максимальная высота среди дочерних узлов для внутреннего узла
 *
 * @note Высота измеряется в количестве уровней, где корень = уровень 1
 */
...
/**
 * @brief Поиск ключа в B-дереве (ДБД)
 * @param root Корень B-дерева
 * @param key Ключ для поиска
 * @return Указатель на узел, содержащий ключ, или nullptr, если не найден
 *
 * @details
 * Выполняет поиск ключа в B-дереве с помощью рекурсивного обхода.
 * Алгоритм:
 * 1. Проверяет ключи текущего узла
 * 2. Если ключ найден - возвращает узел
 * 3. Иначе рекурсивно ищет в дочерних узлах
 *
 * @note В B-дереве ключи внутри узла не обязательно отсортированы,
 * поэтому используется линейный поиск по ключам узла.
 */
