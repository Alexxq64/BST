...
public:
     ...
     *
     * @note Используется для проверки корректности построения B-дерева
     */
     ...
     * @param root Указатель на корень B-дерева
     * @param level Текущий уровень (по умолчанию 0 для корня)
     * @return Взвешенная сумма: ∑(уровень × количество_ключей_на_уровне)
     *
     * @note Используется для вычисления средней глубины ключей в B-дереве
     */
     ...
     *
     * @note Высота измеряется в количестве уровней (корень = уровень 1)
     */
     ...
     *
     * @note Использует рекурсивный обход всех узлов B-дерева
     */
